...
// 프렌딩 탭 — 무료 연습방(FriendingRooms) 목록 전용 페이지.
// 4탭 리뉴얼 6단계: 사이트 첫 화면(/)이 4탭 선택 허브로 바뀌면서 이 내용이 /에서 /friending으로 이동했다.
// 비밀번호 재설정·이메일 인증·회원가입 완료 리다이렉트는 여전히 "/?reset=success" 등 루트를 가리키므로
// 그 배너들은 허브 페이지(src/app/page.tsx)가 그대로 소유한다 — 여기서는 지우지 않는다.
export default async function FriendingPage({ searchParams }: { searchParams: Promise<{ reset?: string; verified?: string; signup?: string }> }) {
